...
class LeadsManageController extends Controller
{
   ...
   public function leadsDashboard(Request $request)
    {
        ...
        $next_leads_query = StudentByAgent::where(DB::raw('next_calling_date'), '>', $currentDateTime)->where('lead_status', '<>', '5')->where('lead_status', '<>', '7')->orderBy('next_calling_date', 'asc') ;
         ...
         $next_leads_query = StudentByAgent::where(DB::raw('next_calling_date'), '>', $currentDateTime)->where('lead_status', '<>', '5')->where('lead_status', '<>', '7')->Where('assigned_to', Auth::user()->id)->orderBy('next_calling_date', 'asc');
        }
        ...
        if ($request->tab1) {
            if($request->id){
                $validator = Validator::make($request->all(), [
                    'first_name' => 'required',
                    // 'email' => 'unique:student_by_agent,email,'.$request->id,
  ...
  public function filterLeads($request)
    {
       ...
       $currentDateTime = now()->toDateTimeString(); // Get current date and time in 'YYYY-MM-DD HH:MM:SS' format
        ...
        if($request->missed_leads){
            ...
            if (Auth::user()->hasRole('Administrator')) 
            {
                    $lead_list->where(DB::raw('next_calling_date'), '<', $currentDateTime)->where('lead_status', '<>', '5')->where('lead_status', '<>', '7')->orderBy('next_calling_date', 'asc');
            }else {
                  $lead_list->where(DB::raw('next_calling_date'), '<', $currentDateTime)->where('lead_status', '<>', '5')->where('lead_status', '<>', '7')->Where('assigned_to', Auth::user()->id)->orderBy('next_calling_date', 'asc');
             }
        }
    ...
    public function lead_list(Request $request, $export = null)
    {
        ...
        $lead_list = $lead_list->paginate(15);
    ...
    public function payment_view(Request $request)
    {
        ...
        $data=[
            ...
            'amount'=>$paymentLink->amount * 2.5/100 + $paymentLink->amount,
    ...
    public function oel_360(Request $request)
    {
        ...
        if($request->first_name){
            $studentData->where('first_name', 'LIKE', "%{$request->first_name}%");
        }
        if($request->email){
            $studentData->where('email', 'LIKE', "%{$request->email}%");
        }
        $studentData =$studentData->where('status_threesixty',1)->where('profile_complete',1)->paginate(12);
    ...
    public function aply_360($id = null)
    {
        ...
        foreach ($paymentStatuses as $masterService => $paymentStatus) {
            ...
            // $student_by_agent_id = StudentByAgent::where('email',$student_email)->pluck('id');
            $checkPayment = PaymentsLink::where('email', $student_email)->where('master_service', $paymentStatus)->latest()->first();
           
            if ($checkPayment) {
                $paymentDone = Payment::where('customer_email', $student_email)->select('payment_status')
                              ->where('fallowp_unique_id', $checkPayment->fallowp_unique_id)->latest()->first();
    ...
    public function store_lead_360(Request $request)
    {
        ...
        if ($request->tab1) {
           ...
           foreach ($application->program_ids as $key => $value) {
                    $id1= Program::where('id', $value)->pluck('id')->first();
                    $program1 = Program::where('id', $value)->pluck('name')->first();
    
                }

       
           $pro_id=$application->program_ids;
            $app_status = $application->{$id1.'_application_status'};
           $app_remarks=$application->{'remarks_'.$id1};
      
           $program1 = Program::whereIn('id', $pro_id)->pluck('name')->implode(', ');
            ...
            $validator = Validator::make($request->all(), [
                'document' => '|mimes:jpeg,png,jpg,gif,pdf,docx|max:6048'
            ]);
            if ($validator->fails()) {
                return response()->json(['status' => false, 'errors' => $validator->errors()], 422);
            }
            ...
            // }else{
                ...
                if($users->hasRole('Administrator')){
                   ...
                   $count= count($request->leadIds);
                    $data = ['status' => true, 'message' => "Franchise Assigned Successfully ! ".$count." leads has been assigned to you"];
