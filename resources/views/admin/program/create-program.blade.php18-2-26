                       ...
                       <div class="form-floating">
                          <select class="form-control " name="school_id" id="school_id" placeholder="University / College Name">
                             ...
                             @foreach ($universities as $item)
                                <option value="{{$item->id}}" {{ old('school_id') == $item->id ? 'selected' : '' }}>{{ implode(' ', array_slice(explode(' ', $item->university_name), 0, 10)) }}</option>
                             @endforeach
                          </select>
                        ...
                        <div class="form-floating">
                            <select class="form-control program_sub_discipline_select" name="program_subdiscipline" id="lead-program-sub-discipline" placeholder="Program Sub Discipline">
                                ...
                                {{-- @foreach ($program_subdiscipline as $item)
                                    <option value="{{$item->id}}" {{ old('program_subdiscipline') == $item->id ? 'selected' : '' }}>{{ implode(' ', array_slice(explode(' ', $item->name), 0, 3)) }}</option>
                                @endforeach --}}
                            </select>
...
<!--                              <option value="NonResidential"  {{ old('programCampus') == 'NonResidential' ? 'selected' : '' }}>NonResidential</option> -->
                    ...
                    <div class="col-4">
    <div class="form-floating">
        <input id="lead-application_closing_date"
               type="date"
               class="form-control date-input"
               placeholder="Application Closing Date"
               value="{{ old('application_closing_date') && !in_array(old('application_closing_date'), ['ASAP','TBD']) ? old('application_closing_date') : '' }}">
        <label for="lead-application_closing_date" class="form-label">
    ...
    <div class="form-check form-check-inline">
        <input class="form-check-input"
               ...
               value="ASAP"
               {{ old('application_closing_date') == 'ASAP' ? 'checked' : '' }}>
    ...
    <div class="form-check form-check-inline">
        <input class="form-check-input"
               ...
               name="application_closing_date"
               value="TBD"
               {{ old('application_closing_date') == 'TBD' ? 'checked' : '' }}>
        <label class="form-check-label">TBD</label>
    </div>

    <!-- Hidden input to store final value -->
    <input type="hidden" name="application_closing_date"
...
$(document).ready(function(){
    ...
    $('.date-input').on('change', function () {
        ...
        $('input[name="application_closing_date"][type="radio"]').prop('checked', false);

        // put date in hidden input
        $('#final_closing_date').val(date);
    });

    // When ASAP/TBD is selected
    $('input[name="application_closing_date"][type="radio"]').on('change', function () {
        let value = $(this).val();

        // clear date field
        $('.date-input').val('');

        // put radio value in hidden input
        $('#final_closing_date').val(value);
    });
